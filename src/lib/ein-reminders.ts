// EIN reminder job — shared logic used by:
//   - scripts/ein-reminders.ts  (local / `pnpm cron:ein-reminders`)
//   - /api/cron/ein-reminders   (Vercel Cron, daily)
//
// Emails clients whose formation is paid but who haven't recorded an EIN yet,
// at most once every 7 days per formation.

import { prisma } from "./prisma";
import { einReminderEmail, sendEmail } from "./email";

const REMINDER_INTERVAL_DAYS = 7;

export async function runEinReminders(): Promise<{ sent: number; eligible: number }> {
  ...
  return { sent, eligible: formations.length };
}
